...
<div class="portlet-content">
	{{ Form::open(array(URL::to('/'), 'class' => 'form label-inline', 'id' => 'edit-admin', 'enctype' => 'multipart/form-data'))}}
	<div class="field">
		<label for="username">Tên đăng nhập<span class="required">*</span></label>
		<input type="text" name="username" id="username" class="large" value="{{ $dataAdmin['username'] }}" required="">
	</div>
	<div class="field">
		<label for="fullname">Họ & Tên</label>
		<input type="text" name="fullname" id="fullname" class="large" value="{{ $dataAdmin['fullname'] }}" required="">
	</div>
	<div class="field">
		<label for="signature">Bút danh<span class="required">*</span></label>
		<input type="text" name="signature" id="signature" class="large" value="{{ $dataAdmin['signature'] }}" required="">
	</div>
	<div class="field">
		<label for="email">Email<span class="required">*</span></label>
		<input type="email" name="email" id="email" class="large" value="{{ $dataAdmin['email'] }}" required="">
	</div>
	<div class="field">
		<label for="datebirth">Ngày sinh</label>
		<input type="text" name="datebirth" class="large" value="{{ date('d-m-Y', $dataAdmin['datebirth']) }}" id="datebirth" required="">
	</div>
	<div class="field">
		...
		<div class="form-input">
			<select name="department" id="department" class="large">
				...
				@foreach($departments as $key => $dep)
					<option value="{{$key}}" <?php echo ($dataAdmin['department'] == $key)?'selected':'' ?> >{{$dep}}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="field">
		...
		<div class="form-input">
			<textarea name="placement" class="large" id="placement">{{$dataAdmin['placement']}}</textarea>
		</div>
	</div>
	<div class="field">
		<label for="summary">Tóm tắt bản thân</label>
		<div class="form-input">
			<textarea name="summary" id="summary" class="large">{{$dataAdmin['summary']}}</textarea>
		</div>
	</div>
	<div class="field">
		<label for="is_approved">Hiển thị</label>
		<select name="is_approved" class="medium" id="is_approved">
			<option value="0" <?php echo ($dataAdmin['is_approved'] == 1)?'selected':'' ?> >Ẩn</option>
			<option value="1" <?php echo ($dataAdmin['is_approved'] == 1)?'selected':'' ?> >Hiển thị</option>
		</select>
	</div>
	<input type="hidden" name="id_usr" value="{{$dataAdmin['id']}}" id="id_usr">
	<div class="field">
